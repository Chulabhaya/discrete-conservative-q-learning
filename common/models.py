...
class DiscreteActorGridVerseObs(nn.Module):
    ...
    def forward(self, states):
        ...
        if len(grid_emb.shape) == 5:
            ...
            unified_grid_emb = torch.flatten(unified_grid_emb, start_dim=2)
            agent_emb = states["agent"]
            x = torch.cat((unified_grid_emb, agent_emb), dim=2)
